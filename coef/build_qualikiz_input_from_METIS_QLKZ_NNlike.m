...
function [] = build_qualikiz_input_from_METIS_QLKZ_NNlike(NNin,runname_qlk,metis_qlkz_path,qlkparams)
...
eval(['!cp ',path_to_METIS_QLKZ,'/parameters_metis_qlkz_NNlike.m ',runpath,'/parameters.m']);
...
ninorm(:,1) = Nex./Nex;
ninorm(:,2) = (NNin.zeff-1)./(Zc.^2-Zc);
ninorm(:,1) = 1-Zc.*ninorm(:,2);

Ani(:,1) = NNin.rlne'./(ninorm(:,1)+Zc.*ninorm(:,2));
Ani(:,2) = Ani(:,1);
...
anis(1:scann,1:size(Ani,2))=1.*ones(scann,size(Ai,2)); %Tperp/Tpar at LFS. Tix defined as Tpar
danisdr(1:scann,1:size(Ani,2))=0.*ones(scann,size(Ai,2)); %d/dr(Tperp/Tpar) at LFS. Tix defined as Tpar

ion_type(1:scann,1:size(Ani,2))=1.*ones(scann,size(Ai,2)); %Tperp/Tpar at LFS. Tix defined as Tpar
...
disp('============== Starts QuaLiKiz run ===============')
    disp(['QuaLiKiz runs on ' num2str(round(qlkparams.numproc)) ' processors'])
    [sout,tout] = unix(sprintf(['module purge \n module load intel/2018 mpi/2018 \n mpiexec -np ' num2str(round(qlkparams.numproc)) ' ./QuaLiKiz']));
toc

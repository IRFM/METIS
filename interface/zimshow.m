function h=zimshow(varargin)
%ZIMSHOW Display image.
%   ZIMSHOW(I,N) displays the intensity image I with N discrete
%   levels of gray. If you omit N, IMSHOW uses 256 gray levels on
%   24-bit displays, or 64 gray levels on other systems.
%
%   ZIMSHOW(I,[LOW HIGH]) displays I as a grayscale intensity
...
%   between display as intermediate shades of gray. ZIMSHOW uses
%   the default number of gray levels. If you use an empty matrix
%   ([]) for [LOW HIGH], ZIMSHOW uses [min(I(:)) max(I(:))]; the
%   minimum value in I displays as black, and the maximum value
%   displays as white.
%
%   ZIMSHOW(BW) displays the binary image BW. Values of 0 display
%   as black, and values of 1 display as white.
%
%   ZIMSHOW(X,MAP) displays the indexed image X with the colormap
%   MAP.
%
%   ZIMSHOW(RGB) displays the truecolor image RGB.
%
%   ZIMSHOW(...,DISPLAY_OPTION) displays the image, calling
...
%
%   ZIMSHOW(x,y,A,...) uses the 2-element vectors x and y to
%   establish a nondefault spatial coordinate system, by
%   specifying the image XData and YData.
%
%   ZIMSHOW(FILENAME) displays the image stored in the graphics
%   file FILENAME. ZIMSHOW calls IMREAD to read the image from the
%   file, but the image data is not stored in the MATLAB
%   workspace. The file must be in the current directory or on
%   the MATLAB path.
%
%   H = ZIMSHOW(...) returns the handle to the image object
%   created by IMSHOW.
%
%   Class Support
%   -------------
...
%   $Revision: 1.3 $  $Date: 2007/01/25 12:30:59 $

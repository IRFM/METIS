...
hform=numcompassform(chemin);
zuifaitnumcompass('init');
zwaitfor(hform)
...
nom  = sprintf('COMPASS-D time step #%g',numchoc);
aide = 'time step edition (function of time)';
...
while (isempty(separatrice))
	...
	h=zuicreefunform('zcompasssepa','sepa_option',1);
	set(h,'name',sprintf('COMPASS-D separatrix parameters #%g',evalin('base','prepare.numchoc')));
	zwaitfor(h)
	code=getappdata(0,'coderetour');
	if ~strcmp(code.action,'validation')
		return
	end
	evalin('base','separatrice = zcompasssepa(prepare.temps,sepa_option);');
	separatrice = evalin('base','separatrice');
end
% dialogue pour les parametres
info         = zcompassacces;
zassignin('base','option',info.valeur);                  
h=zuicreefunform('zcompassacces','option',1);
set(h,'name',sprintf('COMPASS-D shot preparation #%g',evalin('base','prepare.numchoc')));
zwaitfor(h)
code=getappdata(0,'coderetour');
if ~strcmp(code.action,'validation')
	return
...
hdlg = msgbox('CRONOS Input structure in progress','Patience ...','help');
drawnow;
evalin('base','[cr,data,param]=zcompassacces(prepare.numchoc,prepare.chemin,prepare.temps,option,scenario,separatrice);');
if ishandle(hdlg)
	zuicloseone(hdlg);
end
...
if ok == 0
	warndlg(lasterr,'Error COMPASSacces');
	return
end
cr = evalin('base','cr','NaN');
if cr ~= 0
	warndlg('Probleme lors de la lecture des donnees','COMPASSacces');
	return
end
...
function hout=numcompassform(chemin)

% on ferme le formulaire s'il existe
[hfig,h] = zuiformhandle('accesCOMPASS');
if ishandle(hfig)
	zuicloseone(hfig) ;
end
...
col1 = {'text_chemin','text@full','input file path access :',[],'/cgc_data/zineb/data',''};
form{length(form)+1} = {col1};
col1 = {'chemin','edit@full',chemin,42,'input file path access ','','prepare.chemin'};
form{length(form)+1} = {col1};
...
hout=zuicreeform('COMPASS-D shot preparation','accesCOMPASS','zuifaitnumcompass','zuictrlnumcompass',form);
...
col2 = {'text_X','text','temps debut',5,'temps de d�ut de l''intervalle'};
col3 = {'text_Y','text','temps fin  ',5,'temps de fin de l''intervalle'};
form{length(form)+1} = {col1,col2,col1,col3,col1};

col2 = {'valeur_xdeb','edit',tdeb,5,'temps de d�ut de l''intervalle','','prepare.tdeb'};
col3 = {'valeur_xfin','edit',tfin,5,'temps de fin de l''intervalle','','prepare.tfin'};
form{length(form)+1} = {col1,col2,col1,col3,col1};

col2 = {'select_xdeb','radio','selectionner',0,'S�ectionner sur le graphique le temps de d�ut de l''intervalle'};
col3 = {'select_xfin','radio','selectionner',0,'S�ectionner sur le graphique le temps de fin de l''intervalle'};
form{length(form)+1} = {col1,col2,col1,col3,col1};
...
hout=zuicreeform(sprintf('Intervalle de temps pour le choc COMPASS-D #%g',numchoc), ...

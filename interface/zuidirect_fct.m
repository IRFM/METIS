...
if ishandle(hfig)
	...
	evalin('base','tmp = zfrominfo(1,param);unix([getappdata(0,''editeur''),'' '',tmp,'' &'']);', ...
		'fprintf(''Error in zfrominfo :\n%s'',lasterr);');
...
case {'aide'}
	if ishandle(hfig)
		zbrowser('https://wikicronos.partenaires.cea.fr/wiki/index.php/Main_Page');
		zuireset(h.aide)
	end

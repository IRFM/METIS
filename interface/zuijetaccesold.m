...
[hout,hui] = scenarioform(tdeb,tfin,numchoc,dataMSE.tqEFTMx);
...
if ~isempty(dataMSE.tqEFTMx)
  ...
  for kEFTM=1:length(dataMSE.tqEFTMx)
    plot([dataMSE.tqEFTMx(kEFTM) dataMSE.tqEFTMx(kEFTM)],[0 v(4)],':')
  end
...
info         = zjetacces;
option       = info.valeur; 
option.efit  = evalin('base','prepare.mse');
...
function [hout,hui] = scenarioform(tdeb,tfin,numchoc,tmse)
...
col4 = {'text_liste','text','temps MSE  ',5,'temps de mesure MSE'};
...
if ~isempty(tmse)
    sliste =sprintf('Standard|Efit%s',sprintf('|%d',1:length(tmse)));
    col4 = {'select_liste','popup',sliste,2,'temps de mesure MSE,et profil de courant initial',''};
else
    col4 = {'select_liste','popup','Standard|Efit',2,'temps de mesure MSE,et profil de courant initial',''};
end    
...
hout=zuicreeform(sprintf('Intervalle de temps choc JET %g',numchoc), ...
                 'tempsJET','zuifaittjet','zuicontroltjet',form) ;

...
nom  = sprintf('FAST time database, shot #%g',numchoc);
...
while (isempty(separatrice))
	...
	h=zuicreefunform('zfastsepa','sepa_option',1);
	set(h,'name',sprintf('FAST separatrix parameters, #%g',evalin('base','prepare.numchoc')));
	zwaitfor(h)
	code=getappdata(0,'coderetour');
	if ~strcmp(code.action,'validation')
		return
	end
	evalin('base','separatrice = zfastsepa(prepare.temps,sepa_option);');
	separatrice = evalin('base','separatrice');
end
% dialogue pour les parametres
info         = zfastacces;
zassignin('base','option',info.valeur);                  
h=zuicreefunform('zfastacces','option',1);
set(h,'name',sprintf('FAST shot preparation,  #%g',evalin('base','prepare.numchoc')));
zwaitfor(h)
code=getappdata(0,'coderetour');
if ~strcmp(code.action,'validation')
	return
...
drawnow;
evalin('base','[cr,data,param]=zfastacces(prepare.numchoc,prepare.chemin,prepare.temps,option,scenario,separatrice);');
if ishandle(hdlg)
	zuicloseone(hdlg);
end
...
if ok == 0
	warndlg(lasterr,'Errorr FASTacces');
	return
end
cr = evalin('base','cr','NaN');
if cr ~= 0
	warndlg('data access problem','FASTacces');
	return
end
...
if ishandle(hfig)
	...
	if ~isempty(numshot)
		...
		tps=[num2str(tps1) ' at' num2str(tps2)];
...
function hout=numfastform(chemin)
...
hout=zuicreeform('FAST shot preparation','accesFAST','zuifaitnumfast','zuictrlnumfast',form);
...
hout=zuicreeform(sprintf('FAST time interval #%g',numchoc), ...

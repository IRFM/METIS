...
c--------------------------------------------------------------
C Compilation : !fmex zlignemex.f
C--------------------------------------------------------------
...
c       write(*,*) 'debut trajectoire'
	  ...
	  CALL CS(R,phi,Z,BR,BF,BZ,BMOD,BTF)
          do 23 while (Pdeb .le. Pfin .and. IR .lt. nR)       
c	  DO 23 IR=1,nR
	    R   = Rdeb
	    Z   = Zdeb
	    phi = Pdeb
	    ...
	    BZsor(IR) = BZ
	    BPsor(IR) = BF
c	    write(*,*) 'Ir=',ir,Rdeb,Pdeb,Zdeb,'Pfin nR =',pfin,nR	    
	    CALL CS(R,phi,Z,BR,BF,BZ,BMOD,BTF)
c	    write(*,*) 'Bmod=',bmod	    
	    ...
	    Pdeb = Pdeb+BPd*pasR/Rdeb
c	    Pdeb = Pdeb+pasP
c            Rdeb = Rsor(IR) * Bpsor(IR) / BF
c           if  (BZ .ne. 0.0)  then
c                Zdeb = Zsor(IR) - BRsor(IR) / BZsor(IR) *
c     &               (Rdeb - Rsor(IR))
c            else
c               Zdeb = Zsor(IR)
c            endif
c            if (abs(Rsor(IR) -2.04) .gt. 0.01) then
c               Zdeb = Zsor(IR) * abs((Rdeb-2.04) / (Rsor(IR) -2.04))
c            else
c               Zdeb = Zsor(IR) * abs((Rdeb-2.02) / (Rsor(IR) -2.02))
c            endif 
c            Zdeb = Zsor(1) * Bpsor(1) / BF
	    Rdeb = Rdeb+BRd*pasR
	    Rdeb = Rdeb+BRd*pasR
	    Zdeb = Zdeb+BZd*pasR
            IR = IR+1

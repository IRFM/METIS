...
C  [NX,NY,LAMDA,MU,C,FP,DEFRANK,FAIL] = zfitdata(X,Y,F,W,S,MIN,NIN,MINMAX)
C
C entrees :
...
C  S       -> lissage  > 0 [1,1]
C  MIN     -> dimension du fit dans la direction X (ou le temps)
C  NIN     -> dimension du fit dans la dimension Y (ou l'espace)
...
C-----------------------------------------------------------------------
C
      INTEGER*4 MMAX, NXESTMAX, NYESTMAX, LWRK, LIWRK, LCW 
      PARAMETER (MMAX=1000000,NXESTMAX = 10000, NYESTMAX = 100)
      PARAMETER (LWRK=20000000)
      PARAMETER (LIWRK=MMAX + 2*(NXESTMAX-7)*(NYESTMAX-7))
      PARAMETER (LCW = (NXESTMAX-4)*(NYESTMAX-4))
C---------------------------------------------------------------------
...
C
      ...
      REAL*8 LAMDA(NXESTMAX), MU(NYESTMAX)
      REAL*8 X(MMAX), Y(MMAX), F(MMAX), W(MMAX), C(LCW), WRK(LWRK),
     &       WRK1(LWRK)
      REAL*8 S, FP
      integer iopt,kx,ky,nmax,compte
      real*8 xb,xe,yb,ye,eps 
C---------------------------------------------------------------------
...
C  securirte avant recopie
      ...
      IF (mxGetN(PRHS(5)).LT.1) THEN
        CALL mexErrMsgTxt('Il faut donner S')
      ENDIF
      IF (mxGetM(PRHS(6)).LT.1) THEN
        CALL mexErrMsgTxt('Il faut donner NIN')
      ENDIF
      IF (mxGetN(PRHS(6)).LT.1) THEN
        CALL mexErrMsgTxt('Il faut donner NIN')
      ENDIF
      IF (mxGetM(PRHS(7)).LT.1) THEN
        CALL mexErrMsgTxt('Il faut donner MIN')
      ENDIF
      IF (mxGetN(PRHS(7)).LT.1) THEN
        CALL mexErrMsgTxt('Il faut donner MIN')
      ENDIF
...
C  recopie des donnees matlab dans des tableaux fortran
      ...
      CALL mxCopyPtrToReal8(mxGetPr(PRHS(6)),R,1)
      MIN = R
      CALL mxCopyPtrToReal8(mxGetPr(PRHS(7)),R,1)
      NIN = R

C test des dimension X et Y reelles 
      WRITE (*,*) MIN , NIN
      
      NXEST =  MIN + 4
      IF (NXEST.GT.NXESTMAX) THEN
        CALL mexErrMsgTxt('Dimension 1 des tableaux trop grande')
      ENDIF

      NYEST =  NIN  + 4
      IF (NYEST.GT.NYESTMAX) THEN
        CALL mexErrMsgTxt('Dimension 2 des tableaux trop grande')
      ENDIF
      
      IF (NRHS.EQ.8) THEN
          MV = mxGetM(PRHS(8)) * mxGetN(PRHS(8))
          IF (MV.NE.4) THEN
             CALL mexErrMsgTxt('MINMAX doit avoir 4 elements')
          ENDIF
          CALL mxCopyPtrToReal8(mxGetPr(PRHS(8)),MINMAX,4)
          X(M+1) = MINMAX(1)
          X(M+2) = MINMAX(2)
          Y(M+1) = MINMAX(3)
          Y(M+2) = MINMAX(4)
...
C
      ...
      IFAIL = -1
      START  = 'Cold'
c      CALL E02DDF(START,MV,X,Y,F,W,S,NXEST,NYEST,NX,LAMDA,NY,MU,C,FP,
c     #             RANK,WRK,LWRK,IWRK,LIWRK,IFAIL)
c-- remplacement de la fonction NAG par surfit
      iopt = 0
...
c on enleve les elements (x,y,f) dont le poids w est nul
      LX = 0      
      do i=1,M
      	if (W(i).NE.0) then
	   LX = LX +1
      ...
      enddo
      CALL surfit(iopt,LX,X,Y,F,W,xb,xe,yb,ye,kx,ky,S,NXEST,NYEST,
     & nmax,eps,NX,LAMDA,NY,MU,C,FP,WRK,LWRK,WRK1,LWRK,IWRK,LIWRK,IFAIL)
...
C
      ...
      RETURN
      END
      subroutine minvect(tab,dim1,sol)
         implicit none
	 integer dim1,i
	 double precision tab(dim1)
	 ...
	 enddo
      ...
      subroutine maxvect(tab,dim1,sol)
         implicit none
	 integer dim1,i
	 double precision tab(dim1)
	 double precision sol
...
c  if not, interchange x and y once more.
 ...
 930  if(iopt.lt.0) go to 940
      ...
      ny0 = ny
      if (ier .eq. (-2) ) then
       write(*,*) "normal return,weighted least-squared"
      ier = 0  
      endif 
...
c      memoire = w(1)
c-- ajouté :      
c      print *,'W avant sa modif',w
c      do 14 i=2,MV
c      w(i) = memoire
c   14 Continue 
c      print *,'W apres sa modif',w
...
c  are invalid,control is immediately repassed to the calling program.
      ...
      if(m.lt.(kx1*ky1)) then
      print *,'erreur à cause de m',m
      go to 70
      end if
      nminx = 2*kx1
      if(nxest.lt.nminx .or. nxest.gt.nmax) then
c      print *,'erreur à cause de nxest'
      go to 70
      end if
      nminy = 2*ky1
      if(nyest.lt.nminy .or. nyest.gt.nmax) then
c      print *,'erreur à cause de nyest'
      go to 70
      end if
  ...
  10  lwest = ncest*(2+ib1+ib3)+2*(nrint+nest*km2+m*km1)+ib3
      kwest = m+nreg
      if(lwrk1.lt.lwest .or. kwrk.lt.kwest) then 
c      print *,' erreur à cause de lwrk kwrk'
      go to 70
      end if
      if(xb.ge.xe .or. yb.ge.ye) go to 70
      do 20 i=1,m
        if(w(i).le.0.) then 
c	print *,'erreur à cause de W'
	go to 70
	end if
        if(x(i).lt.xb .or. x(i).gt.xe) then
c	print *,'erreur à cause de X'
	go to 70
	end if
        if(y(i).lt.yb .or. y(i).gt.ye) then
c	print *,'erreur à cause de Y'
	go to 70
	end if
  20  continue

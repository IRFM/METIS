...
C-----------------------------------------------------------------------
C
      INTEGER*4 M, N, NW, NQ, IQ(LIQ), IFAIL,IFAIL2,NR
      REAL*8 X(MMAX), Y(MMAX), F(MMAX), XX(MMAX), YY(MMAX), FF(MMAX)
      REAL*8 RQ(LRQ), DFFDX(MMAX) , DFFDY(MMAX)
      real*8 LCELL(MMAX),LNEXT(MMAX),RSQ(MMAX),A(MMAX)
...
c      CALL E01SGF(M,X,Y,F,NW,NQ,IQ,LIQ,RQ,LRQ,IFAIL)
c---------remplacement de la fonction NAG par QSHEP2
      NR = floor(sqrt(M/3.))

      call QSHEP2(M,X,Y,F,NQ,NW,NR, LCELL,LNEXT,XMIN,
     & YMIN,DX,DY,RMAX,RSQ,A,IFAIL2)
...
C
      ...
      IFAIL = 1
c      CALL E01SHF(M,X,Y,F,IQ,LIQ,RQ,LRQ,N,XX,YY,FF,DFFDX,DFFDY,IFAIL)
c------- remplacement de la fonction NAG par QS2GRD
      do 25, i=1,N
      CALL QS2GRD(XX(i),YY(i),M,X,Y,F,NR,LCELL, LNEXT, XMIN,
     & YMIN, DX, DY, RMAX, RSQ, A, Q, QX, QY,IFAIL)
      FF(i)=Q
      DFFDX(i) = QX
      DFFDY(i) = QY

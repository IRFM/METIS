...
C  [NX,NY,LAMDA,MU,C,FP,DEFRANK,FAIL] = zfitdata(X,Y,F,W,S,MINMAX)
C
C entrees :
...
C-----------------------------------------------------------------------
C
      INTEGER   tabm(6),tabn(6),MINVAR, NIN, MV, nv,i,LX
      REAL*8 R, MINMAX(4)
...
C
      ...
      PARAMETER (MMAX=30000,NESTMAX = 1000)
...
C
      ...
      MINVAR = mxGetM(PRHS(1))
      NIN = mxGetN(PRHS(1))


      DO k=1,5
	tabm(k) = mxGetM(PRHS(k))
	tabn(k) = mxGetN(PRHS(k))
      enddo
...
C
      ...
      IFAIL = -1
      START  = 'Cold'
c      write(*,*) 'avant e02ddf'

c-- initialisation de variables pour surfit
      iopt = 0
...
c      CALL E02DDF(START,M,X,Y,F,W,S,NXEST,NYEST,NX,LAMDA,NY,MU,C,FP,
...
c on enleve les elements (x,y,f) dont le poids w est nul
      ...
      enddo
c      print *,'X,W',X(1:LX),W(1:LX)
      CALL surfit(iopt,LX,X,Y,F,W,xb,xe,yb,ye,kx,ky,S,NXEST,NYEST,nmax,
     &     eps,NX,LAMDA,NY,MU,C,FP,WRK,LWRK,WRK1,LWRK,IWRK,LIWRK,IFAIL)
...
c
c
c      subroutine surfitinter(iopt,M,X,Y,F,W,xb,xe,yb,ye,kx,ky,S,NXEST,NYEST,nmax,eps,NX,LAMDA,NY,MU,C,FP,WRK,LWRK,WRK1,LWRK,IWRK,LIWRK,IFAIL)
c      implicit none
      
      
c      call surfitinter(iopt,M,X,Y,F,W,xb,xe,yb,ye,kx,ky,S,NXEST,NYEST,nmax,eps,NX,LAMDA,NY,MU,C,FP,WRK,LWRK,WRK1,LWRK,IWRK,LIWRK,IFAIL)
...
c---- definition des sous routines pour le calcul de la spline bicubique

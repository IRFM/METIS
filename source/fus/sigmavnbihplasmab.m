% cette fonction tabule <sigma*v> pour l'interraction faisceau (p) - plasma (B)
% ref :  D. R. Mikkelsen, NF vol 29 p 1113, 1989
function sigmavnbidplasmab
...
svnbipb = NaN .* ones(length(enbi),length(eti));
%
% reduce masse in eV
...
% boucle sur les temperature plasma
   for l=1:length(eti)
      enbic         = enbi(k);
      etic          = eti(l);
      vb            = sqrt(2 .* phys.e .*  enbic./ phys.mp);
      vth           = sqrt(2 .* phys.e .*  etic ./ phys.ua ./ 11.0093054);
      svcb          = trapz(v,svb2int_pb(v,vth,vb));
      svcb           = svcb ./ (vb .* vth .* sqrt(pi)) ;  
      svnbipb(k,l)  = svcb;
      fprintf(' svnbitd(%g,%g) = %g  (m^3*s)\n',vb,vth,svcb);
   end
end
save sigmav_nbi_p_plasma_b enbi eti svnbipb

...
function [salpha,emean] = zbpi0_bp(nH,ti,pnbi_th,taus_nbi,e0nbi,ecritnbi,ftnbi,flag)

persistent svs
if isempty(svs)
   svs = load('sigmav_nbi_b_plasma_p');
end
...
vc          = sqrt(2 .* phys.e .*  ecritnbi ./ phys.ua ./ 11.0093054);
v0          = sqrt(2 .* phys.e .*  e0nbi ./ phys.ua ./ 11.0093054);
inde        = min(find(e0nbi <= svs.enbi));
if isempty(inde)
   inde = length(svs.enbi);
end
vi          = sqrt(2 .* phys.e .*  svs.enbi(1:inde)' ./ phys.ua ./ 11.0093054);
ut          = ones(size(nH));
ue          = ones(size(vi));
...
svnbibp     = svs.svnbibp(1:inde,:);
svm         = tsplinet(ones(size(svnbibp,1),1)*svs.eti(:)',svnbibp,ones(size(svnbibp,1),1)*min(ti',max(svs.eti)))';
salpha      = 4 .* pi .* nH .* trapz(vi,fn0 .* svm .* (ut *vi) .^ 2,2) * 3;  % reaction with 3 alpha particles
% securite
salpha(pnbi_th <=0) = 0;

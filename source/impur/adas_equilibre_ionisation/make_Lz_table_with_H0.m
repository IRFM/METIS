...
other = other.tabmat;
simple = load('Lz_zave_simple.mat');
simple = simple.tabmat;
name_other = fieldnames(other);
% liste des elements
[A,Z,name] = chargemasse;
missing_element = {};
% start to compute D0/D+ equilibrium 
[Te1,Lz1,Zave1,Z2ave1,Zl,rep_H1,year_out,source_out,quality] = compute_lz('H',year,'adas');            
[Te2,Lz2,Zave2,Z2ave2,Zl,rep_H2,year_out,source_out,quality] = compute_lz('T',year,'adas');    
if all(Te1 == Te2)
    Te_D = Te1;
...
%  	  [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz('Be',93,'web');      
	  ...
	  name_loc = 'He';
	  [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz('He',year,'adas',nH0ne);      
	  ...
	  name_loc = 'Si';
	  [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz('Si',[96,96],'web',nH0ne);      
      elseif strcmp(name{k},'W')
  	  name_loc = 'W';
  	  [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz('W',[50,0],'web',nH0ne);      
       elseif strcmp(name{k},'Fe')
  	  name_loc = 'Fe';
  	  [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz('Fe',[85,89],'web',nH0ne);      
        elseif strcmp(name{k},'Cl')
  	  name_loc = 'Cl';
  	  [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz('Cl',89,'adas',nH0ne);    
    else
	  name_loc = name{k};
	  fprintf('Element: %s\n',name_loc);
	      disp('Trying ADAS source')
	      [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz(name{k},year,'adas',nH0ne);
	  if isempty(Lz)
	      disp('Trying open ADAS source')
	      [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz(name{k},year,'web',nH0ne);      
	  end
	  if isempty(Lz)
	      disp('Trying Mattioli data')
	      [Te,Lz,Zave,Z2ave,Zl,rep,year_out,source_out,quality] = compute_lz(name{k},year,'mattioli',nH0ne);      	  
	  end
      end
      if ~isempty(Lz)
	tabmat.(name{k}).A = A(k);
	...
	subplot(2,2,1)
	if strcmp(name{k},'W')
	    [teW,lzW] =  lzw_Putterich;
	    loglog(teW./1e3,lzW,'k-.')
	else
	    loglog(simple.(name{k}).data(:,1),simple.(name{k}).data(:,2),'k-.')
	end
	hold on
	...
	title(sprintf('%s @ %s/%d (with D0)',name{k},sl,tabmat.(name{k}).year{1}));
	xlabel('Te (keV)');
	ylabel('Lz (W m^3)')
	set(gca,'xlim',[1e-3,50]);
	if strcmp(name{k},'W')
	    legend('Putterich','CRONOS/METIS with D0','PROCESS','Location','Best');
	else
	    legend('CRONOS/METIS without D0','CRONOS/METIS with D0','PROCESS','Location','Best');
	end
	subplot(2,2,2)
	loglog(simple.(name{k}).data(:,1),simple.(name{k}).data(:,3),'k-.')
	hold on
	if ~isempty(strmatch(name_loc,name_other,'exact'))
	...
	subplot(2,2,4)
	loglog(simple.(name{k}).data(:,1),simple.(name{k}).data(:,4),'k-.')
	hold on
	loglog(tabmat.(name{k}).data(:,1),tabmat.(name{k}).data(:,4),'b');
	xlabel('Te (keV)');
	ylabel('<Z^2>')	
	legend('CRONOS/METIS without D0','CRONOS/METIS with D0','Location','Best');
	set(gca,'xlim',[1e-3,50]);
	set(gcf,'PaperOrientation','landscape')
	drawnow
	hgsave(sprintf('Lz__H0_%s',name{k}));
	print(gcf, '-dpng', sprintf('Lz_H0_%s',name{k}));

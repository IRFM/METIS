...
function [salpha,emean] = zbpi0(nD,ti,pnbi_th,taus_nbi,e0nbi,ecritnbi,ftnbi,flag)

persistent svs
if isempty(svs)
   svs = load('sigmav_nbi_t_plasma_d');
end
...
vi          = sqrt(2 .*e .*  svs.enbi(1:inde)' ./ mp ./ 3);
ut          = ones(size(nD));
ue          = ones(size(vi));

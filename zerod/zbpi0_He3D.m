...
function [salpha,emean] = zbpi0_He3D(nD,ti,pnbi_th,taus_nbi,e0nbi,ecritnbi,ftnbi,flag)

persistent svs
if isempty(svs)
    svs = load('sigmav_nbi_he3_plasma_d');
end


% fonction de distribution des ions He3 issu de l'injection de neutre
...
vc          = sqrt(2 .*e .*  ecritnbi ./ mHe3);
v0          = sqrt(2 .*e .*  e0nbi ./ mHe3);
inde        = min(find(e0nbi <= svs.enbi));
if isempty(inde)
   inde = length(svs.enbi);
end
vi          = sqrt(2 .*e .*  svs.enbi(1:inde)' ./ mHe3);
ut          = ones(size(nD));
ue          = ones(size(vi));
...
svnbihe3d     = svs.svnbihe3d(1:inde,:);
svm         = tsplinet(ones(size(svnbihe3d,1),1)*svs.eti,svnbihe3d,ones(size(svnbihe3d,1),1)*min(ti',max(svs.eti)))';
salpha      = 4 .* pi .* nD .* trapz(vi,fn0 .* svm .* (ut *vi) .^ 2,2);
% securite
salpha(pnbi_th <=0) = 0;

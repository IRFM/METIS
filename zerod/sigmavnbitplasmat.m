...
function sigmavnbitplasmat

% plot de verification 
ecm  = linspace(0,4.7e6,1001);
sddt  =  tt_cross_section(ecm) ./ 1e-31;
figure
loglog(ecm,sddt,'r');
xlabel('Ecm (eV)');
ylabel('Sigma (mb)');
...
% boucle sur les temperature plasma
   for l=1:length(eti)
      ...
      svcb          = trapz(v,svb2int(v,vth,vb,2));
      %warning off
      %svc           = quad('svb2int',la,lb,1e-28,0,vth,vb,1);
      %warning on
      %svc           = svc ./ (vb .* vth .* sqrt(pi));  
      svcb           = svcb ./ (vb .* vth .* sqrt(pi));  
      svnbitt(k,l)  = svcb;
      fprintf(' svnbitt(%g,%g) = %g  @ %g   (m^3*s)\n',vb,vth,svcb,svcb);
   end
end
save sigmav_nbi_t_plasma_t enbi eti svnbitt

...
%  O. Suater et al, PoP vol 9 (2002) p 5140-5141 (errat)
%  S. Koh et al, PoP vol 19 (2012)   p 072505-1 to 072505-12 
%  il y a sans doute un bug dans la formule beta_epsi du papier
function jboot = zsauter0d_Koh(x,te,ti,ne,ni,q,zeff,zion,Raxe,ft,epsi,dpsidx,F,modeh,fgg,model_zi)
...
% correction de la fraction de pieges effective au bord
ft(:,end) = max(0,(1 - 0.4 ./ Zib(:,end) .^ 4 .* exp(0)) .* ft(:,end));

% factuer de correction au denominateur de fonctions X
beta_epsi = real((epsi - 0.44) .^ 0.7); % cette formule cree des ecart important avec NClass sur les profiles !
alpha_z   = (-Zib .^ 2 + 5.998 .* Zib - 4.981) ./ (4.294 .* Zib .^2 -14.07 .* Zib + 12.61);
delta     = 0.55 .* Zib .^ 0.2 .* (tanh(3.2 .* beta_epsi .* (epsi .^ (3/2) ./ nustare) .^ 1.4 ./ Zib .^ alpha_z) + ...
            (1 - exp(-nustare ./ 0.1)) .* tanh(2.2 .* beta_epsi .* epsi .^ 2.8 .* nustare .^ 0.1 ./ Zib .^ alpha_z)); 
delta(:,1) = 0;

% X = f^{31}_{t,eff}
X              = ft .* (1 + delta) ./ (1 + (1-0.1*ft).*sqrt(nustare) + 0.5*(1-ft).*nustare./Zib);

L31            = (1+(1.4 ./ (Zib+1))).*X - (1.9 ./ (Zib+1)).*X.^2 + (0.3 ./ (Zib+1)).*X.^3 + (0.2 ./ (Zib+1)).*X.^4;

% X2 = f^{32_ee}_{t,eff}
X2             = ft .* (1 + delta) ./ (1 + 0.26*(1-ft).*sqrt(nustare) + 0.18*(1-0.37*ft).*nustare./sqrt(Zib));
% X3 = f^{32_ei}_{t,eff}
X3             = ft .* (1 + delta) ./ (1 + (1+0.6*ft).*sqrt(nustare) + 0.85*(1-0.37*ft).*nustare.*(1+Zib));
...
% X4 = f^{34}_{t,eff}
X4             = ft .* (1 + delta) ./ (1 + (1 - 0.1*ft).*sqrt(nustare) + 0.5*(1-0.5*ft).*nustare./Zib);
L34            = (1+(1.4 ./ (Zib+1))).*X4 - ...
                 (1.9 ./ (Zib+1)).*X4.^2  + ...
                 (0.3 ./ (Zib+1)).*X4.^3  + ...
                 (0.2 ./ (Zib+1)).*X4.^4;

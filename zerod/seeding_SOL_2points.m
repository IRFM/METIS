...
name='Range of seeding (in %) :';
numlines=1;
defaultanswer={'0','10','301'};
answer=inputdlg(prompt,name,numlines,defaultanswer);
if ~isempty(answer)
        seeding_list = linspace(max(0,min(str2num(answer{1}),str2num(answer{2}))), ...
                              min(100,max(str2num(answer{1}),str2num(answer{2}))), ...
                              min(3001,max(11,str2num(answer{3}))))';
	nbt     = length(seeding_list);
else
	nbt     = 301;
	seeding_list = linspace(0,100,nbt)';
end
...
k    = 6;
subplot(k,1,1)
semilogy(seeding_list,zerod.nebord,seeding_list,nelim/1e19);
ylabel('10^{19} m^{-3}');
...
subplot(k,1,2)
semilogy(seeding_list,tebord,seeding_list,telim);
ylabel('eV');
legend('T_{e,LCFS}','T_{e,t}')
z0loglin(gca);
subplot(k,1,3)
semilogy(seeding_list,qpl./1e6,seeding_list,qpl_in_loc./1e6,seeding_list,delta_qpl_rad./1e6,seeding_list,delta_qpl_neutral./1e6);
ylabel('MW/m^2');
legend('q_{//,t}','q_{//,LCFS}','\delta(q_{//,rad,div })','\delta(q_{//,neutral,div })');
z0loglin(gca);
subplot(k,1,4)
semilogy(seeding_list,fmom,seeding_list,mach_target,seeding_list,fcond);
legend('f_{mom}','M_t','f_{cond} * f_{pe}');
z0loglin(gca);
subplot(k,1,5)
semilogy(seeding_list,zeff_div,seeding_list,gamma);
legend('Z_{eff} div','\gamma');
z0loglin(gca);
subplot(k,1,6)
semilogy(seeding_list,prad_loc./1e6,seeding_list,pradsol_loc./1e6);
legend('Prad core (line)','Prad SOL (line + brem)');
ylabel('MW');
xlabel('Seeding (in percent)');
z0loglin(gca);
joint_axes(h,k);
drawnow
edition2

...
while isempty(scaling)
	...
	d95     = 0.47;
	ip    = 21.3e6;
	nbar  = 1.0 .* 1e20 .* (ip / 1e6) ./ (pi.* a .^ 2);
	plh   = 0e6;
	picrh = 0e6;
	pecrh = 0e6;
	pnbi  = 110e6;
	zeff  = (1-0.005) + 18 .^ 2 .* 0.0038;
	hmore = 1.5;
	iso   = 1;
	ftnbi = 0.5;
	ane   = 1.1;
	...
	sepa_option.rxup      = 0.53;     % upper triangularity (minor radius unit)
	sepa_option.zxup      = 1.8;    % upper altitude X point (minor radius unit)
	sepa_option.apup      = 0;       % upper separatrix angle (R,X)  (LFS, degrees)
	sepa_option.amup      = 0;       % upper separatrix angle (-R,X) (HFS, degrees)
	sepa_option.ra        = 8.15;       % major radius R0 (m) [6.2]
	sepa_option.za        = 0;       % altitude of the magnetic axis (m) [0.9]
	sepa_option.a         = 8.15/3        % minor radius (m) [2]
	sepa_option.rxdo      = 0.66;     % lower triangularity (minor radius unit)
	sepa_option.zxdo      = 2.12;       % lower altitude X point (minor radius unit)
	sepa_option.apdo      = 90 - 67.92;   % lower separatrix angle (R,X)  (LFS, degrees)
	sepa_option.amdo      = 67.92;   % lower separatrix angle (-R,X)  (HFS, degrees)
	sepa_option.b0        = 14./1.05;      % magnetic field at R0
	sepa_option.delta     = 1.6;
...
switch mode
case   'P'
	z0dinput = zerod_scalaire(temps,b0,R,a,K,d,ip,nbar,plh,picrh,pecrh,pnbi,zeff,xece,hmore .* (scaling == 0)+ (scaling ~= 0),iso,ftnbi,0, ...
                          'gaz',3,'frhe0',0,'tauhemul',5,'ane',4,'vane',ane, ...
			  'scaling',scaling,'l2hscalin',0,'modeh',1,'l2hmul',0,'fpped',fpped, ...
			  'xiioxie',2,'kishape',3,'qdds',0.95,'kidds',3,'vloop',0,'runaway',0,'modeboot',1,'vref',0, ...
			  'zeff',1,'zmax',18,'zimp',4,'rimp',1,'matthews',0, ...
			  'frad',frad,'rw',rw,'angle_ece',90,'synergie',1, ...
			  'sens',1,'angle_nbi',70,'einj',einj,'rtang',0,'lhmode',3,'etalh',0.8, ...
			  'npar0',2,'freqlh',5,'wlh',a/2,'xlh',0,'dlh',0,'fwcd',1, ...
			  'mino','T','cmin',1,'nphi',25,'freq',65,'sitb',0,'tae',0, ...
			  'smhd',0,'tmhd',inf,'rip',0,'fprad',fprad,'li',1);
case 'N'
	z0dinput = zerod_scalaire(temps,b0,R,a,K,d,ip,nbar,plhop,picrh,pecrh,pnbi-plhop,zeff,xece,ones(size(hmore)),iso,ftnbi,0, ...
                          'gaz',3,'frhe0',0,'tauhemul',5,'ane',3,'vane',1, ...
			  'scaling',0,'l2hscalin',0,'modeh',1,'l2hmul',0,'fpped',0.5, ...
			  'xiioxie',0,'kishape',3,'qdds',0.95,'kidds',3,'vloop',0,'runaway',1, ...
			  'modeboot',1,'vref',0, ...
			  'zeff',1,'zmax',18,'zimp',4,'rimp',1,'matthews',0, ...
			  'frad',1,'rw',rw,'angle_ece',90,'synergie',0, ...
			  'sens',1,'angle_nbi',65,'einj',einj,'rtang',0,'lhmode',3,'etalh',0.8, ...
			  'npar0',2,'freqlh',5,'wlh',a/2,'xlh',0,'dlh',0,'fwcd',1, ...
			  'mino','T','cmin',1,'nphi',25,'freq',72,'sitb',3,'tae',1, ...
			  'smhd',0,'tmhd',0,'rip',0,'fprad',fprad,'li',1);
...
fprintf('Plh = %g, Picrh = %g, Pecrh = %g, Pnbi = %g, Pline = %g, Pbrem = %g, Pcyclo = %g\n',zs.plh(end-2)/1e6, ...
        zs.picrh(end-2)/1e6,zs.pecrh(end-2)/1e6,zs.pnbi(end-2)/1e6,zs.prad(end-2)/1e6+zs.pradsol(end-2)/1e6,zs.pbrem(end-2)/1e6, zs.pcyclo(end-2) ./1e6);

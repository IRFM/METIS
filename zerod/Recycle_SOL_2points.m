...
if ~isempty(answer)
        Recycle_list = linspace(max(0,min(str2num(answer{1}),str2num(answer{2}))), ...
                              min(1,max(str2num(answer{1}),str2num(answer{2}))), ...
                              min(3001,max(11,str2num(answer{3}))))';
	nbt     = length(Recycle_list);
else
	nbt     = 301;
	Recycle_list = linspace(0,1,nbt)';
end
...
k    = 6;
subplot(k,1,1)
semilogy(Recycle_list,zerod.nebord,Recycle_list,nelim/1e19);
ylabel('10^{19} m^{-3}');
legend('n_{e,LCFS}','n_{e,t}')
z0loglin(gca);
title(sprintf('METIS : %s@%d/ Two Point model (ramp of Recycling parameter, use GUI set of parameters)',post.z0dinput.machine,post.z0dinput.shot));
subplot(k,1,2)
semilogy(Recycle_list,tebord,Recycle_list,telim);
ylabel('eV');
legend('T_{e,LCFS}','T_{e,t}')
z0loglin(gca);
subplot(k,1,3)
semilogy(Recycle_list,qpl./1e6,Recycle_list,qpl_in_loc./1e6,Recycle_list,delta_qpl_rad./1e6,Recycle_list,delta_qpl_neutral./1e6);
ylabel('MW/m^2');
legend('q_{//,t}','q_{//,LCFS}','\delta(q_{//,rad,div })','\delta(q_{//,neutral,div })');
z0loglin(gca);
subplot(k,1,4)
semilogy(Recycle_list,fmom,Recycle_list,mach_target,Recycle_list,fcond);
legend('f_{mom}','M_t','f_{cond} * f_{pe}');
z0loglin(gca);
subplot(k,1,5)
semilogy(Recycle_list,zeff_div,Recycle_list,gamma);
legend('Z_{eff} div','\gamma');
z0loglin(gca);
subplot(k,1,6)
semilogy(Recycle_list,prad_loc./1e6,Recycle_list,pradsol_loc./1e6);

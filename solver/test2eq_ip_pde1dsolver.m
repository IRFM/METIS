...
disp('La concentration est en interpretatif');
...
cm=zeros(L+1,K);                 % matrice des concentrations
cm(1,fix(K/2)+1)=c0;             % le concentre est mis au centre du tube
...
for l=1:L
	...
	FP0=cat(2,c(l+1,:)',zeros(size(c(l+1,:)')));
	mode=[1,0];

...
case 'TS'
  ...
  if ~isempty(bile.pel) 
    plotprof(gca,bile.times,xbile,bile.pel,'linestyle','*');
  end
  title(sprintf('Pel shot %d',fix(param.from.shot.num)))
  ylabel('Pa')
  if ~isempty(t1) & ~isempty(bile.pel)
    legend('CRONOS','reference','fit mesure');
  elseif ~isempty(t1) & isempty(bile.pel)
     legend('CRONOS','fit mesure');
  else 
    legend('CRONOS');
  end
  subplot(2,2,2)
  plotprof(gca,t,x,real(data.prof.pion),'color','r');
  if ~isempty(bile.pion)  
    plotprof(gca,bile.times,xbile,bile.pion,'linestyle','o');
    pion2 = bile.nefit  .* (bile.ti_ne * ones(1,21)) .* bile.nifit  .* param.phys.e .* 1e3 ;
    plotprof(gca,bile.times,xbile,pion2,'linestyle','*');
    pion3 = bile.tefit .* bile.nifit  .* param.phys.e .* 1e3 ;
    plotprof(gca,bile.times,xbile,pion3,'linestyle','-.','color','m');
  end
  title('Pion ')
  ylabel('Pa')
  if ~isempty(bile.pion)
    legend('CRONOS','\alpha*T_e*N_i','\alpha*N_e*N_i','= Ni*Te');
  end

  subplot(2,2,3)
  plotprof(gca,t,x,data.prof.te/1e3,'color','r');
  if ~isempty(bile.tefit)
    plotprof(gca,bile.times,xbile,bile.tefit,'linestyle','*','color','b');
  ...
  ylabel('keV')
  xlabel('rho')
  if ~isempty(t1) & ~isempty(bile.tefit)
    legend('CRONOS','fit','Thomson','ECE','reference');
  elseif ~isempty(t1) & isempty(bile.tefit)
    legend('CRONOS','reference');
  end

  subplot(2,2,4)
  plotprof(gca,t,x,data.prof.ti/1e3,'color','r');
  if ~isempty(bile.ti_te)
    ti1 = bile.tefit  .* (bile.ti_te * ones(1,21)) ;
    plotprof(gca,bile.times,xbile,ti1,'linestyle','o');
    ti2 = bile.nefit  .* (bile.ti_ne * ones(1,21)) ;
    plotprof(gca,bile.times,xbile,ti2,'linestyle','*');
    plotprof(gca,bile.times,xbile,bile.tefit,'linestyle','-.','color','m');
  end
  if ~isempty(t1)
  ...
  ylabel('keV')
  xlabel('rho')
  if ~isempty(bile.pion) & isempty(t1)
    legend('CRONOS','\alpha*T_e','\alpha*N_e','= Te');
  elseif ~isempty(bile.pion) & ~isempty(t1)
	 legend('CRONOS','\alpha*T_e','\alpha*N_e','= Te','reference');
  end
%
% trace temporelle
%
  if ~isempty(t1)
    ...
    h1=subplot(4,1,1);
    plot(t,data.prof.te(:,fin1)/1e3,'b',t1,jeux1.data.prof.te(:,fin1)/1e3,'b--',bile.times,bile.tefit(:,iround(bile.rhofit,0.2)),'ro')
    axis(vgraph1)
    ylabel('keV')
    set(h1,'xticklabel','')
    ...
    h2=subplot(4,1,2);
    plot(t,data.prof.te(:,fin2)/1e3,'b',t1,jeux1.data.prof.te(:,fin2)/1e3,'b--',bile.times,bile.tefit(:,iround(bile.rhofit,0.4)),'ro')
    ...
    h3=subplot(4,1,3);
    plot(t,data.prof.te(:,fin3)/1e3,'b',t1,jeux1.data.prof.te(:,fin3)/1e3,'b--',bile.times,bile.tefit(:,iround(bile.rhofit,0.6)),'ro')
    ...
    h4=subplot(4,1,4);
    plot(t,data.prof.te(:,fin4)/1e3,'b',t1,jeux1.data.prof.te(:,fin4)/1e3,'b--',bile.times,bile.tefit(:,iround(bile.rhofit,0.8)),'ro')
...
case 'DIIID'
  ...
  plotprof(gca,t,x,data.prof.pe,'color','r');
  if ~isempty(t1)
     plotprof(gca,t1,x,jeux1.data.prof.pe,'linestyle','--','color','b');
  end
  plotprof(gca,t,post.thomson.x,post.thomson.te.*post.thomson.ne.*param.phys.e,'linestyle','o','color','r');
  
  title(sprintf('Pel shot %d',fix(param.from.shot.num)))
  ylabel('Pa')
  if ~isempty(t1)
    legend('CRONOS','reference','Thomson');
  else
    legend('CRONOS','Thomson');
  end
  subplot(2,2,2)
  ...
  plotprof(gca,diiidprof.t,diiidprof.x,diiidprof.Te,'linestyle','--','color','b');
  plotprof(gca,data.gene.temps,post.thomson.x,post.thomson.te/1e3,'linestyle','o','color','b');

  title('Te ')
  ylabel('keV')
  xlabel('x')
  if ~isempty(t1)
    legend('CRONOS','reference','diiidprof','Thomson');
  else
    legend('CRONOS','diiidprof','Thomson');        
  end      
  subplot(2,2,4)
  plotprof(gca,t,x,data.prof.ti/1e3,'color','r');
  if ~isempty(t1)
  ...
  plotprof(gca,diiidprof.t,diiidprof.x,diiidprof.Ti,'linestyle','x','color','r');
  if ~isempty(t1)
    legend('CRONOS ','reference','diiidprof')
  else
    legend('CRONOS ','diiidprof')
  end
  ylabel('keV')
  xlabel('rho')
  set(gcf,'ResizeFcn','')

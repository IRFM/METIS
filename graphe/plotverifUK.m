...
h1=subplot(4,1,1);
if ~isempty(bile.ip)
  plot(t,data.exp.ip(inde)/1e6,'ro',tt,data.gene.ip(ind)/1e6,'b--');
  if exist('jeux1','var')
    hold on
%    plot(tt1,jeux1.data.gene.ip(ind1)/1e6,'c.')
    plot(tt1,jeux1.data.gene.ip(ind1)/1e6,'b')
end
    set(h1,'xticklabel','')
  axis(vgraph1)
  set(h1,'xtick',tickgr)
...
h2=subplot(4,1,2);
if ~isempty(bile.ip)
  plot(t,data.exp.vloop(inde),'ro', ...
       tt,medfilt1(data.gene.vsurf(ind),5),'b--');
  set(h2,'xticklabel','')
  ylabel('Vloop (V)')
  set(gca,'ylim',[0 2])
  if exist('jeux1','var')
    hold on
    plot(tt1,jeux1.data.gene.vloop(ind1),'b')
    hold off
  end
  if exist('jeux1','var')
    taille = get(gca,'fontsize');
...
h3=subplot(4,1,3);
if ~isempty(bile.ip)
  plot(bile.times,bile.beli,'ro',tt,data.gene.betap(ind)+data.gene.li(ind)/2,'b--')
  if exist('jeux1')
    hold on
    plot(tt1,jeux1.data.gene.betap(ind1)+jeux1.data.gene.li(ind1)/2,'b')
    taille = get(gca,'fontsize');
    set(gca,'fontsize',12)  
%    legend(gca,'exp.','CRONOS','reference',2)
    set(gca,'fontsize',taille)
...
ind95 =length(param.gene.x);
if ~isempty(bile.ip)
  plot(t,data.exp.qa(inde),'ro',tt,data.prof.q(ind,ind95),'b--')
  if exist('jeux1')
    hold on
    plot(tt1,jeux1.data.prof.q(ind1,ind95),'b')
    taille = get(gca,'fontsize');
    set(gca,'fontsize',12)  
%    legend(gca,'exp.','CRONOS','reference',2)
    set(gca,'fontsize',taille)
...
vgraph1 = [tt(1)-3 tt(imax) 0 max(data.exp.ip(inde)/1e6)*1.3];
vgraph2 = [tt(1)-3 tt(imax) -1 2];
if ~isempty(jettemp.betap)
  mvgraph3 = max(jettemp.betap(1:indjetli)+jettemp.li(1:indjetli)/2);
else
  mvgraph3 = max(jettemp.li(1:indjetli));
end
vgraph3 = [tt(1)-3 tt(imax) 0.5 mvgraph3*1.2];
mvgraph4 = max(jettemp.li(1:indjetli));
vgraph4 = [tt(1)-3 tt(imax) 0.5 mvgraph4*1.2];
ind95   =length(param.gene.x);
vgraph5 = [tt(1)-3 tt(imax) 2 max(data.prof.q(ind,ind95))*1.2];
...
h2=subplot(5,1,2);
plot(t,data.exp.vloop(inde),'r',tt,data.gene.vloop(ind),'b');
if exist('jeux1','var')
  hold on
  plot(tt1,jeux1.data.gene.vloop(ind1),'r.')
  hold off

function [Tisol,Wd,Piq,Fnor] = zTi(data,param)
...
for k=1:length(temps)
  ...
  sv     = 2.33e-14/2*exp(-18.76./Ti.^0.33333)./Ti.^0.666667;
...
h = findobj(0,'type','figure','tag','Tin');
if isempty(h)
       h=figure('tag','Tin');
else
       figure(h);
...
subplot(2,1,1)
ind = find(Tisol(:,1)*1e3 > 200);
plot(temps(ind),Tisol(ind,1)*1e3,'r+',temps,data.prof.ti(:,1))
legend('Ti neutron x=0','Ti Cronos x = 0')
title('Ti profile deduced from Ti neutron and Ti bragg')

subplot(2,2,3)

contourf(x,temps(ind),Tisol(ind,:)*1e3,linspace(20,max(data.prof.ti(:,1)),10))
colorbar
title('Ti neutron')
axis([0 1 param.gene.tdeb param.gene.t])

subplot(2,2,4)
indg=min(find(data.gene.temps>12 & Tisol(:,1) > 0));
plot(x,Tisol(indg,:)*1e3,x,data.prof.ti(indg,:),'ro')
legend('Ti neutron','Ti cronos')

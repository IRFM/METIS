...
vgraph1 = [0 tmax 0 max(data.gene.ip(ind)/1e6)*1.1];
vgraph2 = [0 tmax 0 2];
vgraph3 = [0 tmax 0.5 2];
ind95   =length(param.gene.x);

vgraph4 = [0 tmax 2 max(data.prof.q(ind,ind95))*1.2];
...
h1=subplot(5,1,1);
vgraph1  = [0 tmax 0 max([phybmax pfcicon*1.1 nbarmax])];
...
set(gca,'fontsize',12)   
axis(vgraph1)
[valt,indt] = max(data.gene.paddhyb/1e6);
tgra        = t(indt);
%text('units','data','position',[tgra valt],'string','PLH')
title('#28205')
grid
...
if exist('jeux1','var')
    hold on
    plot(tt1,jeux1.data.gene.ip(ind1)/1e6,'r:')
    plot(tt1,jeux1.data.gene.ini(ind1)/1e6,'b.')
    hold off
end
if exist('bile','var')
    hold on
    plot(bile.times,bile.ip,'r--')
    hold off
end
...
if exist('bile','var')
    hold on
    plot(bile.times,bile.betap+bile.li/2,'b--')
    hold off
end

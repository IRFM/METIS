...
vgraph2 = [tmin tmax -0.1 0.1];
vgraph3 = [tmin tmax 0.2 0.5];
...
if ~isempty(jeux1)
  plot(t,data.gene.paddhyb/1e6,'b',...
  t1,jeux1.data.gene.nbar/1e19,'r--')
else
...
h1=subplot(5,1,2);
vgraph1 = [tmin tmax 0.4 max(data.gene.ip(ind)/1e6)*1.1];
if isempty(jeux1)
  plot(tt,data.gene.ip(ind)/1e6,'r', ...
       tt,data.gene.ini(ind)/1e6,'b--');
else
  plot(tt,data.gene.ini(ind)/1e6,'b');
...
if ~isempty(jeux1)
    hold on
    plot(tt1,jeux1.data.gene.ini(ind1)/1e6,'bo')
    hold off
end

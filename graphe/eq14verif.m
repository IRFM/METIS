...
%subplot(2,1,2)
plot(param.gene.x,data.prof.dpsidt(ii,:),param.gene.x,terme1+terme2+terme3,'ro')
...
plot(param.gene.x,data.prof.jmoy(ii,:),param.gene.x,data.source.totale.j(ii-1,:)+data.prof.epar(ii,:)./data.neo.eta(ii,:),param.gene.x,jequierr)
legend('jmoy','jni+E/eta','j erreur eq 14')
subplot(3,1,3)
semilogy(param.gene.x,abs(data.prof.dpsidt(ii,:)-terme1-terme2-terme3)./ ...
    abs(diff(data.prof.psi(ii,[1,end]))),'ro',x,abs(jequierr./data.prof.jmoy(ii,:)),'b')

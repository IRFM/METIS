% METIS_FEEQS_LINK: gateway between FEEQS and METIS
%-------------------------------------------------------------
% fonction Matlab2013b: metis_feeqs_link,set_default_parameters
%
% This function recieve data from FEEQS (plasma LCFS, ...) ,compute METIS in mode evolution
% and return input profiles and data for FEEQS equilibrium
%
% syntax:
%
%   initialisation:
%       [metis_data4feeqs,internal_state,sepa_metis] = metis_feeqs_link([],feeqs_data4metis,kinetic_control,parameters);
%     or
%       metis_data4feeqs = metis_feeqs_link([],feeqs_data4metis,kinetic_control,parameters);
%
%   time evolution:
%       [metis_data4feeqs,internal_state] = metis_feeqs_link(internal_state,feeqs_data4metis,kinetic_control,parameters);
%    or
%       metis_data4feeqs = metis_feeqs_link('one_time',feeqs_data4metis,kinetic_control,parameters);
%
% input:
%
%     internal_state       = METIS internal state data structure (structure generated by the function, must not be change externally)
%     feeqs_data4metis    = data sent from FEEQS to METIS (structure)
%     kinetic_control      = optionnal structure for kinetic control (struture)
%     parameters           = configuration parameters of metis_feeqs_link
%
% with
%
...
%    	internal_state.raw_z0dinput      =  raw input from METIS simulation before change in metis_feeqs_link
%    	internal_state.feeqs_data4metis =  copy of last data structure from FEEQS
%    	internal_state.kinetic_control   =  copy of last kinetic_control structure (can be empty)
%    	internal_state.parameters        =  parameters of metis_feeqs_link
%    	internal_state.metis_data4feeqs =  last data from METIS provided to FEEQS
%
%   feeqs_data4metis must contain:
%
%	feeqs_data4metis.time	 = current time  [1] (s)
%	feeqs_data4metis.Ip     = plasma current [1] (A)
%       feeqs_data4metis.Bp_error_vacuum  = error magnetic field, i.e. vaccum poloidal magnetic field for breakdown computation  [1] (T)
%                                            (if = 0, brekdown model is turn off)
%
%       LCFS described by R and Z point list (vectors of same length in meter)
%  		feeqs_data4metis.R_LCFS  [M]
%  		feeqs_data4metis.Z_LCFS  [M]
%          with M >= 5
%
%	feeqs_data4metis.Psi	  = poloidal flux vector Psi from magnetic axis to LCFS (Wb / 2pi, maximum value on magnetic axis) [N]
%	feeqs_data4metis.Raxe	  = radius of magnetic axis versus PSI[n] (m)
%	feeqs_data4metis.K	  = fux surface elongation K = (max(Z) - min(Z)) ./ (max(R) - min(R)) versus psi [N]
%          with N >= 2
%
%   kinetic_control:
%
...
%
%     parameters.save  =  root name of files  (one by call) used to save all internal variables on purpose of debuging
%
...
% output:
%
%      metis_data4feeqs      = data sent from METIS to feeqs (structure)
...
%
%  metis_data4feeqs content:
%
%       metis_data4feeqs.time       = current time [1] (s)
%  	metis_data4feeqs.ip         = plasma current [1] (A)
%  	metis_data4feeqs.li         = internal inductance (li_3) [1]
%  	metis_data4feeqs.betap      = beta poloidal (including fast particle contribution) [1]
%  	metis_data4feeqs.psi        = poloidal flux profile [21] (Wb/(2pi))
%  	metis_data4feeqs.q          = safety factor [21]
%  	metis_data4feeqs.rho        = toroidal rho (sqrt(Phi/(pi*B0)) [21] (m)
%  	metis_data4feeqs.phi        = toroidal flux [21] (Wb)
%  	metis_data4feeqs.epar       = parallele electric field [21) (V/m)
%  	metis_data4feeqs.ptot       = total pressure [21] (Pa)
%  	metis_data4feeqsd.ptotdpsi  = dPtot/dPsi  [21] (first input profile for Grad Shafranov equation)
%  	metis_data4feeqs.fdia       = diamagnetic function(R*Bphi) [21] (T.m)
%  	metis_data4feeqs.df2dpsi    = d(fdia^2)/dPsi [21] (second input profile for Grad Shafranov equation)
%  	metis_data4feeqs.jphi       = surface averaged toroidal current (<J_phi/R>/<1/R>) [21] (A/m^2)
%
...
%         metis_data4feeqs = metis_feeqs_link
%
%      for k=1:10
%       	metis_data4feeqs = metis_feeqs_link('one_time')
%      end
%
% function wrote by J-F Artaud
%-----------------------------------------------------------------------
%
function [metis_data4feeqs,internal_state,sepa_metis] = metis_feeqs_link(internal_state,feeqs_data4metis,kinetic_control,parameters)

% for testing
%persistent test_flux
...
if (nargout < 2) && isempty(internal_state)
    if isappdata(0,'METIS_FEEQS_INTERNAL_STATE')
        rmappdata(0,'METIS_FEEQS_INTERNAL_STATE');
    end
    setappdata(0,'METIS_FEEQS_INTERNAL_STATE',[]);
    internal_state = getappdata(0,'METIS_FEEQS_INTERNAL_STATE');
elseif nargout < 2
    if ~isappdata(0,'METIS_FEEQS_INTERNAL_STATE')
        setappdata(0,'METIS_FEEQS_INTERNAL_STATE',[]);
    end
...
% check data from FEEQS
% feeqs_data4metis must contain:
% * time (s)
%	feeqs_data4metis.time	 [1]
% * plasma current (A)
%	feeqs_data4metis.Ip	 [1]
%
% * error magnetic field, i.e. vaccum poloidal magnetic field for breakdown computation (T).
%     feeqs_data4metis.Bp_error_vacuum
%
% * LCFS described by R and Z point list (vectors of same length in meter)
%  		feeqs_data4metis.R_LCFS  [m]
%  		feeqs_data4metis.Z_LCFS  [m]
...
%	feeqs_data4metis.Psi	 [n]
% * radius of magnetic axis versus PSI (m)
%	feeqs_data4metis.Raxe	 [n]
% * fux surface elongation K = (max(Z) - min(Z)) ./ (max(R) - min(R)) versus psi
%	feeqs_data4metis.K	 [n]
%   with n >= 2
if isempty(feeqs_data4metis)
    disp('INFO: FEEQS data structure for METIS is empty');
end
switch method
    case 'init'
        if ~isempty(feeqs_data4metis) && isfield(feeqs_data4metis,'time')
            time  = feeqs_data4metis.time(end);
            ip_in = feeqs_data4metis.Ip(end);
        else
...
% synchronise METIS input and FEEQS
index = find(z0dinput.cons.temps >= time,1);
if isempty(index)
    index = length(z0dinput.cons.temps);
end

% handle LCFS from FEEQS
if isfield(feeqs_data4metis,'R_LCFS') && isfield(feeqs_data4metis,'Z_LCFS')
    R0  = (min(feeqs_data4metis.R_LCFS) + max(feeqs_data4metis.R_LCFS)) ./ 2;
    z0 = (min(feeqs_data4metis.Z_LCFS) + max(feeqs_data4metis.Z_LCFS)) ./ 2;
    %
    KH = sort(unique(convhull(feeqs_data4metis.R_LCFS,feeqs_data4metis.Z_LCFS)));
    if (length(KH) ~= length(feeqs_data4metis.R_LCFS))
        index_full = 1:length(feeqs_data4metis.R_LCFS);
        Rsepa = feeqs_data4metis.R_LCFS(KH);
        Zsepa = feeqs_data4metis.Z_LCFS(KH);
        feeqs_data4metis.R_LCFS = interp1(KH,Rsepa,index_full,'linear');
        feeqs_data4metis.Z_LCFS = interp1(KH,Zsepa,index_full,'linear');
        indbad_lcfs = find(~isfinite(feeqs_data4metis.R_LCFS) | ~isfinite(feeqs_data4metis.Z_LCFS));
        if ~isempty(indbad_lcfs)
            feeqs_data4metis.R_LCFS(indbad_lcfs) = [];
            feeqs_data4metis.Z_LCFS(indbad_lcfs) = [];
        end
    end
    geo_metis    = zerod_get1t(z0dinput.geo,index);
    if isfield(feeqs_data4metis,'Raxe')
        loc.d0 = feeqs_data4metis.Raxe(1) - feeqs_data4metis.Raxe(end);
    else
	loc.d0 = 0;
    end
    rb0          = geo_metis.R .* geo_metis.b0;
    if length(feeqs_data4metis.R_LCFS) ~= parameters.nb_pts_lcfs
	[Rsepa_out,Zsepa_out] = z0reshape_sepa(geo_metis,loc,feeqs_data4metis.R_LCFS,feeqs_data4metis.Z_LCFS - (min(feeqs_data4metis.Z_LCFS) + max(feeqs_data4metis.Z_LCFS)) ./ 2,5,1,parameters.nb_pts_lcfs);
        Zsepa_out = Zsepa_out - (min(Zsepa_out) + max(Zsepa_out)) / 2;
    else
	Rsepa_out = feeqs_data4metis.R_LCFS;
	Zsepa_out = feeqs_data4metis.Z_LCFS;
    end
    ...
    %figure;plot(feeqs_data4metis.R_LCFS,feeqs_data4metis.Z_LCFS,'.r',Rsepa_out,Zsepa_out + geo_metis.z0,'-.b',sepa_metis.Rsepa,sepa_metis.Zsepa + geo_metis.z0,'k');
else
    geo_metis    = zerod_get1t(z0dinput.geo,index);
...
switch method
	...
	elseif isfield(feeqs_data4metis,'R_LCFS') && isfield(feeqs_data4metis,'Z_LCFS')
...
switch method
    case 'init'
        if ~isempty(feeqs_data4metis) && isfield(feeqs_data4metis,'Psi')
            internal_state.psi_offset = feeqs_data4metis.Psi(end);
        else
    ...
    otherwise
        ...
        if ~isempty(feeqs_data4metis) && isfield(feeqs_data4metis,'Psi')
            %ip_star   = internal_state.data_zerod.ip .* (1 + tanh((internal_state.profil0d.psi(end) - (feeqs_data4metis.Psi(end) - internal_state.psi_offset)) ./ norme_psi));

            % new version of ip_star with inductance correction
            norme_psi = Lp ./ 2 ./ pi .* internal_state.data_zerod.ip;
%              %fprintf('norme: ');disp([norme_psi,internal_state.profil0d.psi(1) - internal_state.profil0d.psi(end)]);
%              if isfield(memoire,'time')
%                  Cor_psi   = (internal_state.profil0d.psi(end) - (feeqs_data4metis.Psi(end) - internal_state.psi_offset)) ./ norme_psi;
%                  index     = max(find(memoire.time < time));
%                  if isempty(index)
%  		    Cor_lp = 0;
%  		    delta_ip_max = 0;
%                  else
%  		    Cor_lp = (Lp - memoire.Lp(index)) ./ Lp;                
%  		    delta_ip_max = parameters.max_abs_dip_dt .* (time - memoire.time(index));
%                  end
%                  fprintf('Correction: ');disp([Cor_psi,-Cor_lp,Cor_psi - Cor_lp]);
%                  % maximum variation of ip 
%                  % as antialiasing
%  		ip_star   = internal_state.data_zerod.ip .* (1 + tanh(Cor_psi - Cor_lp));
%  		if abs(ip_star - internal_state.data_zerod.ip) > delta_ip_max
%  		    fprintf('Delta Ip: ');disp([abs(ip_star - internal_state.data_zerod.ip),delta_ip_max])
%  		    ip_star   = internal_state.data_zerod.ip .* (1 + tanh(Cor_psi));
%  		end 
%              else
		ip_star   = internal_state.data_zerod.ip .* (1 + tanh((internal_state.profil0d.psi(end) - (feeqs_data4metis.Psi(end) - internal_state.psi_offset)) ./ norme_psi));
%            end
	    fprintf('Psi: ');disp([internal_state.profil0d.psi(1),internal_state.profil0d.psi(end),feeqs_data4metis.Psi(1) - internal_state.psi_offset,feeqs_data4metis.Psi(end) - internal_state.psi_offset]);
...
% replace z0dinput METIS data with FEEQS prescribe data
cons1t.temps  = time;
exp0d1t.temps = time;
%
...
% edge poloidal flux
if isfield(feeqs_data4metis,'Psi')
    cons1t.flux = feeqs_data4metis.Psi(end) - internal_state.psi_offset;
    
    % for testing
    %      if isempty(test_flux) && (parameters.ipfluxmode == 1)
    %  	test_flux = cons1t.flux;
    %      elseif ~isempty(test_flux) && (parameters.ipfluxmode == 1)
    %  	cons1t.flux = test_flux;
    %      end
    if isfield(feeqs_data4metis,'L_ext')
        cons1t.L_ext    = feeqs_data4metis.L_ext;
    else
        cons1t.L_ext    = Lp/2/pi;
    end
    if isfield(feeqs_data4metis,'Psi_ext')
        cons1t.Psi_ext    = feeqs_data4metis.Psi_ext;
    else
        cons1t.Psi_ext    =  NaN;
    end
    cons1t.flux_ip  = cons1t.flux -  cons1t.L_ext .* cons1t.ip; % external flux variation is missing
...
% error field for breakdown, if it is provided by FEEQS
if isfield(feeqs_data4metis,'Bp_error_vacuum')
    option.berror = feeqs_data4metis.Bp_error_vacuum;
end
% adding constraint on K_min, if it is provided by FEEQS
if isfield(feeqs_data4metis,'K')
    cons1t.K_min  = min(feeqs_data4metis.K);
end
% adding contraint on Shafranov shift at magnetic axis, if it is provided by FEEQS
if isfield(feeqs_data4metis,'Raxe')
    cons1t.d0_ext = feeqs_data4metis.Raxe(1) - feeqs_data4metis.Raxe(end);
end
...
switch method
    case 'init'
        ...
        if ~isfield(z0dstruct.z0dinput.cons,'L_ext')
                if isfield(feeqs_data4metis,'L_ext')
                   z0dstruct.z0dinput.cons.L_ext    = feeqs_data4metis.L_ext * ones(size(z0dstruct.z0dinput.cons.temps));
                else
	...
	if ~isfield(z0dstruct.z0dinput.cons,'Psi_ext')
		if isfield(feeqs_data4metis,'Psi_ext')
		    z0dstruct.z0dinput.cons.Psi_ext    = feeqs_data4metis.Psi_ext * ones(size(z0dstruct.z0dinput.cons.temps));
		else
...
% generate output for FEEQS
metis_data4feeqs.time       = zs.temps;
metis_data4feeqs.ip         = zs.ip;
metis_data4feeqs.vloop      = zs.vloop;
metis_data4feeqs.li         = zs.li;
metis_data4feeqs.betap      = zs.betaptot;
metis_data4feeqs.w          = zs.w;
metis_data4feeqs.xli        = profil.xli;
metis_data4feeqs.psi        = profil.psi;
metis_data4feeqs.q          = profil.qjli;
metis_data4feeqs.rho        = profil.rmx;
metis_data4feeqs.phi        = profil.phi;
metis_data4feeqs.epar       = profil.epar;
metis_data4feeqs.ptot       = profil.ptot;
metis_data4feeqs.dptotdpsi  = profil.dptotdpsi;
metis_data4feeqs.fdia       = profil.fdia;
metis_data4feeqs.df2dpsi    = profil.df2dpsi;
metis_data4feeqs.jphi       = profil.jli;
metis_data4feeqs.surface    = cumtrapz(profil.xli,profil.spr,2);
metis_data4feeqs.volume     = cumtrapz(profil.xli,profil.vpr,2);
metis_data4feeqs.rmx        = profil.rmx;
metis_data4feeqs.C2         = profil.C2;
metis_data4feeqs.Raxe       = profil.Raxe;
metis_data4feeqs.ri         = profil.ri;
metis_data4feeqs.r2i        = profil.r2i;
metis_data4feeqs.psin       = (metis_data4feeqs.psi - metis_data4feeqs.psi(:,1) * ones(1,size(metis_data4feeqs.psi,2))) ./ ...
                              ((metis_data4feeqs.psi(:,end) - metis_data4feeqs.psi(:,1)) * ones(1,size(metis_data4feeqs.psi,2)));

% alternative derivation of dPtotdPsi and df2dpsi
% improved value on magnetic axis
psid1             = pdederive(profil.xli,metis_data4feeqs.psi,0,2,2,1);
psid1(:,end)      = -(2*pi) .* mu0 .* metis_data4feeqs.rmx(:,end) .* metis_data4feeqs.ip ./ metis_data4feeqs.C2(:,end);
% dspidx = 0 au centre et d2psidx2 doit etre nul au bord pour que ip soit defini precisement
psid2    = pdederive(profil.xli,metis_data4feeqs.psi,1,0,2,2);
dpdpsi   = pdederive(profil.xli,metis_data4feeqs.ptot,0,1,2,1) ./ min(-eps,psid1);
dpdpsi(psid1 > -eps) = 0;
...
dpdx_1 = (metis_data4feeqs.ptot(:,2) - metis_data4feeqs.ptot(:,1)) ./ profil.xli(2) .^ 2;
dpdpsi_1 = - 2 .* dpdx_1 ./ (metis_data4feeqs.fdia(:,1) ./ metis_data4feeqs.Raxe(:,1)) .* metis_data4feeqs.q(:,1) ./ metis_data4feeqs.rmx(:,end) .^ 2;
dpdpsi(:,1) = max(dpdpsi(:,1),dpdpsi_1);
df2dpsi  = 2 .* mu0 .* (max(0,metis_data4feeqs.jphi) .* metis_data4feeqs.ri - dpdpsi)./ metis_data4feeqs.r2i;
% direct derivation
for k=1:size(metis_data4feeqs.psi,1)
    dpdpsi_direct(k,:) =   pdederive(metis_data4feeqs.psi(k,:),metis_data4feeqs.ptot(k,:),2,2,2,1);
    df2dpsi_direct(k,:) =   pdederive(metis_data4feeqs.psi(k,:),metis_data4feeqs.fdia(k,:) .^ 2,2,2,2,1);
end
jphi_rebuilt = (df2dpsi .* metis_data4feeqs.r2i ./ (2 .* mu0)  + dpdpsi) ./ metis_data4feeqs.ri;
if 0   
    figure;
    subplot(2,2,1)
    plot(metis_data4feeqs.psin',metis_data4feeqs.dptotdpsi','r',metis_data4feeqs.psin',dpdpsi','b',metis_data4feeqs.psin',dpdpsi_direct','g');
    xlabel('\psi_n');
    ylabel('dPd\psi');
    ...
    plot(metis_data4feeqs.psin',metis_data4feeqs.dptotdpsi','.r',metis_data4feeqs.psin',dpdpsi','.b',metis_data4feeqs.psin',dpdpsi_direct','.g');
    subplot(2,2,2)
    plot(metis_data4feeqs.psin',metis_data4feeqs.df2dpsi','r',metis_data4feeqs.psin',df2dpsi','b',metis_data4feeqs.psin',df2dpsi_direct','g');
    xlabel('\psi_n');
    ylabel('dF^2d\psi');
    hold on
    plot(metis_data4feeqs.psin',metis_data4feeqs.df2dpsi','.r',metis_data4feeqs.psin',df2dpsi','.b',metis_data4feeqs.psin',df2dpsi_direct','.g');
    subplot(2,2,3);
    plot(metis_data4feeqs.psin',metis_data4feeqs.jphi','r',metis_data4feeqs.psin',jphi_rebuilt','b');
    xlabel('\psi_n');
    ylabel('J_\phi');
    hold on
    plot(metis_data4feeqs.psin',metis_data4feeqs.jphi','.r',metis_data4feeqs.psin',jphi_rebuilt','.b');
...
metis_data4feeqs.dptotdpsi = dpdpsi;
metis_data4feeqs.df2dpsi   = df2dpsi;
...
internal_state.feeqs_data4metis  = feeqs_data4metis;
internal_state.kinetic_control   = kinetic_control;
internal_state.parameters        = parameters;
internal_state.metis_data4feeqs  = metis_data4feeqs;
...
logtrace.psi_axe_eq     = feeqs_data4metis.Psi(1) - internal_state.psi_offset;
logtrace.psi_lcfs_eq    = feeqs_data4metis.Psi(end) - internal_state.psi_offset;
...
% mode without internal_state input/output
if nargout < 2
    setappdata(0,'METIS_FEEQS_INTERNAL_STATE',internal_state);
end

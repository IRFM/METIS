...
function coresource = mapcore_sources_nbicd_imas(z0dstruct,data_zerod,profil0d,texte_diary,error_flag,sigma_B0_eff,coresource)

%% IDENTIFIER of the source
coresource.identifier.name  = 'nbi';
coresource.identifier.index = 2;
coresource.identifier.description='NBICD source of power, particles, current and momentum from METIS model';
% null profile
...
for k=1:length(profil0d.temps)
	...
	profiles_1d.electrons.energy             = max(0,profil0d.pnbi(k,:) - profil0d.pnbi_ion(k,:));
        profiles_1d.total_ion_energy             = profil0d.pnbi_ion(k,:);
        profiles_1d.momentum_tor                 = profil0d.rot_nbi(k,:);
        profiles_1d.j_parallel                   = profil0d.jnbicd(k,:);

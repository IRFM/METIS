...
function coresource = mapcore_sources_eccd_imas(z0dstruct,data_zerod,profil0d,texte_diary,error_flag,sigma_B0_eff,coresource)


%% IDENTIFIER of the source
coresource.identifier.name  = 'ec';
coresource.identifier.index = 3;
coresource.identifier.description='ECCD source of power and current from METIS model';
% null profile
...
for k=1:length(profil0d.temps)
	...
	profiles_1d.electrons.energy             = profil0d.pecrh(k,:);

        profiles_1d.total_ion_energy             = zz;
        profiles_1d.momentum_tor                 = zz;
        profiles_1d.j_parallel                   = profil0d.jeccd(k,:);

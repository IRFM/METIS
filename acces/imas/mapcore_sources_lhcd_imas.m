...
function coresource = mapcore_sources_lhcd_imas(z0dstruct,data_zerod,profil0d,texte_diary,error_flag,sigma_B0_eff,coresource)

%% IDENTIFIER of the source
switch z0dstruct.z0dinput.option.lhmode
case 5
	coresource.identifier.name  = 'ec';
	coresource.identifier.index = 3;
	coresource.identifier.description='second ECCD source of power and current from METIS model';
otherwise
	coresource.identifier.name  = 'lh';
	coresource.identifier.index = 4;
	coresource.identifier.description='LHCD source of power, current and momentum from METIS model';
end
% null profile
...
for k=1:length(profil0d.temps)
	...
	profiles_1d.electrons.energy             = profil0d.plh(k,:);

        profiles_1d.total_ion_energy             = zz;
        profiles_1d.momentum_tor                 = profil0d.rot_lh(k,:);
        profiles_1d.j_parallel                   = profil0d.jlh(k,:);

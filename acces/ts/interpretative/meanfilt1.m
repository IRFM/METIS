function [y,ymin,ymax,ymean,ystd] = meanfilt1(x,n,blksz)
...
%       $Revision: 1.3 $  $Date: 2006/01/13 14:48:58 $
...
for i=1:blksz:nx
    ...
    ymin(i:min(i+blksz-1,nx)) = min(xx);
    ymax(i:min(i+blksz-1,nx)) = max(xx);
    ymean(i:min(i+blksz-1,nx)) = mean(xx);
    ystd(i:min(i+blksz-1,nx)) = std(xx);
end

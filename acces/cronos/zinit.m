...
%
%
gene.psiequi   = 0;              % 0,1,2 -> no copy / systematic copy / copy if different {2}; for FBE , if = 1  implement predictor/corrector for Ip as boundary condition of current diffusion equation.
...
fonction.idn        = 'znemo';          % function for calculation of NBI sources
...
equi.free.psi_plasma        = vt;  % plasma poloidal flux (Wb/2/pi)
equi.free.psi_ext           = vt;  % poloidal flux due to all but plasma (Wb/2/pi)
equi.free.L_plasma          = vt;  % total inductance of the plasma (H/2/pi)
equi.free.L_int             = vt;  % internal inductance of the plasma (H/2/pi)
equi.free.L_ext             = vt;  % extrenal inductance of the plasma (H/2/pi)
...
cons.flux_ip    = vt;             % hybrid boundary condition at the edge for  poloidal flux diffusion (Wb/2/pi)
...
mode.cons.psi        = vt;            % boundary condition for current diffusion : Ip (0), Vloop (1), edge psi (2), Ip predictition with predictor/corrector in free boundary equilibrium (FBE) mode (3), self consitant mode with Ip predictition with predictor/corrector with FBE (4), hybrid boundary condition with FBE (5) and self consitant mode with with predictor/corrector with FBE (6).
...
sg.nfast          = mt;    % fast ion density (definition: energy above 1.5*Ti(0)), unity = m^-3
...
sg.synergie       = mt;    % synergy factor for LHCD & ECCD
sg.sigmasyn       = mt;    % synergy condictivity reads : sigmasyn = (jtot- jrf - johm)/epar (ohm^-1 * m^-1)

...
C***FIRST EXECUTABLE STATEMENT  INITDS
      IF (NOS .LT. 1) CALL MEXERRMSGTXT(
     & 'SLATEC INITDS Number of coefficients is less than 1')
C
      ERR = 0.
      DO 10 II = 1,NOS
        I = NOS + 1 - II
   ...
   10 CONTINUE
C
   20 IF (I .EQ. NOS) CALL MEXERRMSGTXT (
     &'SLATEC INITDS Chebyshev series too short for specified accuracy')
      INITDS = I
C
      RETURN
      END

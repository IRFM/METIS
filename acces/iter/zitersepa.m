function  sepa = zitersepa(temps,option)

% declaration des parametres
if nargin <=1 
	valeur.rx      = 5.09;     % rayon du point x (m)
	valeur.zx      = -3.36;    % hauteur du point x (m)
	valeur.ap      = 22.46;    % angle de la separatrice au point x (R,X)  (cote LFS, degres)
	valeur.am      = 67.92;    % angle de la separatrice au point x (-R,X) l'horizontale(cote HFS, degres)
	valeur.ra      = 6.2;      % grand rayon geometrique (m) [6.2]
	valeur.a       = 2;        % petit rayon geometrique (m) {2,1.85}
	valeur.k       = 1.85;     % elongation (b/a) {1.85,1.97}
	valeur.d       = 0.49;     % triangularite moyenne (Cl + Cu) / 2*a {0.49,0.58}
	valeur.b0      = 5.3;      % champ toroidal a R = 6.2 m {5.3,5.18}

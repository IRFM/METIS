...
C***FIRST EXECUTABLE STATEMENT  DCSEVL
      ...
      FIRST = .FALSE.
      IF (N .LT. 1) CALL MEXERRMSGTXT (
     &  'SLATEC DCSEVL NUMBER OF TERMS .LE. 0')
      IF (N .GT. 1000) CALL MEXERRMSGTXT (
     & 'SLATEC DCSEVL NUMBER OF TERMS .GT. 1000')
      IF (ABS(X) .GT. ONEPL) CALL MEXERRMSGTXT (
     & 'SLATEC DCSEVL X OUTSIDE THE INTERVAL (-1,+1)')
C
      B1 = 0.0D0
      B0 = 0.0D0
      TWOX = 2.0D0*X

...
c pointeurs mex   
      CRONOSINT plhs(*), prhs(*),mxGetPr, mxcreatedoublematrix
      integer   mxgetN, mxgetM
...
c pointeurs et tailles      
      ...
      data two/2./,one/1./,rho/1./ 
c      data rkappax1/1.687/,ptdeltax1/0.466/,psip1dg/0./,psim1dg/0./,
c     & rkappax2/2.001/,ptdeltax2/0.568/,psip2dg/22.46/,psim2dg/67.92/
**********************************************************************

C---------------------------------------------------------------------
C GESTION DES ENTREES     
C entree
C      write(*,*) 'debut sepratrice'
...
c internal part
       ...
       sizeOUT = 125*4
C x
C      write(*,*) 'debut ecriture de x et y'
      plhs(1) = mxcreatedoublematrix(125*4,1,0)
      x_pr = mxGetPr(plhs(1))
      call mxCopyReal8ToPtr(x,x_pr,125*4)
C x
      plhs(2) = mxcreatedoublematrix(125*4,1,0)
      y_pr = mxGetPr(plhs(2))

% function returning data retrieving from SAL server.
% 
% syntax:
%
%    [data,raw_text] = sal_get(path,[authorisation,revision,type])
%
% input:
...
%    type                 = 'full' = retrieved data or 'summary' = data description only
%
% output:  data structure  (see SAL documentation for  details)
%
...
%
function [data,t] = sal_get(data_path,authorisation_machine,revision,type)

if nargin < 1
    error('syntax: data = sal_get(data_path,[authorisation_machine,revision,type])');
...
url = sprintf('https://%s%s?revision=%d&object=%s&auth=%s', ...
      authorisation_machine.server,data_path,revision,type,authorisation_machine.token);
try
    t = webread(url,opt);
catch
    if ~isempty(strfind(lasterr,'NOT FOUND'))
        t = sprintf('return NOT FOUND for : %s\n',data_path);
        data = [];
        return;
    else
        % built wegt querry
        ...
        [s,t] = system(sprintf('wget -q -O - --header="Host: %s" --header="Authorization: Bearer %s" "https://%s%s?revision=%d&object=%s&auth=%s"', ...
                authorisation_machine.server,authorisation_machine.token,authorisation_machine.server,data_path,revision,type,authorisation_machine.token));
        
        % error handling
        if s ~= 0
            t = sprintf('data access error for: %s (%s)\n',data_path,t);
            data = [];
            return;
        end
    end
